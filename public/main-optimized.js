...
function initializeAuth() {
  if (typeof window.AuthManager !== "undefined") {
    ...
    // Set up a listener for auth state changes without creating recursion
    if (window.AuthManager.onAuthStateChange) {
      window.AuthManager.onAuthStateChange(() => {
        // Update local UI without calling AuthManager again
        updateLocalAuthUI();
      });
    }
  }
...
// Optimized authentication UI update (local only, no recursion)
function updateAuthUI() {
  updateLocalAuthUI();
}

// Update local auth UI without calling AuthManager
function updateLocalAuthUI() {
  const authButtons = document.getElementById("authButtons");
  const userMenu = document.getElementById("userMenu");
  const userGreeting = document.getElementById("userGreeting");

  // Get current user from AuthManager if available
...
const AnimationManager = {
  ...
  setupIntersectionObserver() {
    ...
    document
      .querySelectorAll(".testimonials .testimonial-card")
      .forEach((card) => {
