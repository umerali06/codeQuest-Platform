<?php
/**
 * Seed Coding Challenges Only to Appwrite
 * This script focuses only on seeding the challenges collection
 */
...
echo "🎯 Seeding Coding Challenges to Appwrite\n";
echo "========================================\n\n";
...
try {
    ...
    // Define the coding challenges to seed
    $challenges = [
        [
        ...
        [
            ...
            'starter_code_css' => 'body { font-family: Arial, sans-serif; text-align: center; padding: 50px; }\nbutton { padding: 10px 20px; font-size: 16px; cursor: pointer; }',
            'starter_code_js' => '// Add click event listener to button\n// Change the title text when clicked',
            'solution_code_html' => '<!DOCTYPE html>\n<html>\n<head>\n    <title>Interactive Page</title>\n</head>\n<body>\n    <h1 id="title">Hello World</h1>\n    <button id="changeBtn">Change Text</button>\n    <script>\n        document.getElementById("changeBtn").addEventListener("click", function() {\n            document.getElementById("title").textContent = "Text Changed!";\n        });\n    </script>\n</body>\n</html>',
            'solution_code_css' => 'body { font-family: Arial, sans-serif; text-align: center; padding: 50px; }\nbutton { padding: 10px 20px; font-size: 16px; cursor: pointer; }',
            'solution_code_js' => 'document.getElementById("changeBtn").addEventListener("click", function() {\n    document.getElementById("title").textContent = "Text Changed!";\n});',
            'is_active' => true
        ],
        [
            'lesson_id' => 'javascript-intermediate',
            ...
            'starter_code_css' => 'form { display: flex; flex-direction: column; gap: 10px; max-width: 300px; margin: 50px auto; }\ninput { padding: 10px; border: 1px solid #ccc; border-radius: 4px; }\nbutton { padding: 10px; background: #007bff; color: white; border: none; border-radius: 4px; cursor: pointer; }\n.error { color: red; font-size: 14px; }',
            'starter_code_js' => '// Add form validation logic\n// Check username length, email format, password strength',
            'solution_code_html' => '<!DOCTYPE html>\n<html>\n<head>\n    <title>Form Validation</title>\n</head>\n<body>\n    <form id="myForm">\n        <input type="text" id="username" placeholder="Username" required>\n        <input type="email" id="email" placeholder="Email" required>\n        <input type="password" id="password" placeholder="Password" required>\n        <button type="submit">Submit</button>\n    </form>\n    <div id="errors"></div>\n    <script>\n        document.getElementById("myForm").addEventListener("submit", function(e) {\n            e.preventDefault();\n            validateForm();\n        });\n        \n        function validateForm() {\n            const username = document.getElementById("username").value;\n            const email = document.getElementById("email").value;\n            const password = document.getElementById("password").value;\n            const errors = [];\n            \n            if (username.length < 3) errors.push("Username must be at least 3 characters");\n            if (!email.includes("@")) errors.push("Please enter a valid email");\n            if (password.length < 6) errors.push("Password must be at least 6 characters");\n            \n            if (errors.length === 0) {\n                alert("Form is valid!");\n            } else {\n                document.getElementById("errors").innerHTML = errors.map(err => `<div class="error">${err}</div>`).join("");\n            }\n        }\n    </script>\n</body>\n</html>',
            'solution_code_css' => 'form { display: flex; flex-direction: column; gap: 10px; max-width: 300px; margin: 50px auto; }\ninput { padding: 10px; border: 1px solid #ccc; border-radius: 4px; }\nbutton { padding: 10px; background: #007bff; color: white; border: none; border-radius: 4px; cursor: pointer; }\n.error { color: red; font-size: 14px; }',
            'solution_code_js' => 'document.getElementById("myForm").addEventListener("submit", function(e) {\n    e.preventDefault();\n    validateForm();\n});\n\nfunction validateForm() {\n    const username = document.getElementById("username").value;\n    const email = document.getElementById("email").value;\n    const password = document.getElementById("password").value;\n    const errors = [];\n    \n    if (username.length < 3) errors.push("Username must be at least 3 characters");\n    if (!email.includes("@")) errors.push("Please enter a valid email");\n    if (password.length < 6) errors.push("Password must be at least 6 characters");\n    \n    if (errors.length === 0) {\n        alert("Form is valid!");\n    } else {\n        document.getElementById("errors").innerHTML = errors.map(err => `<div class="error">${err}</div>`).join("");\n    }\n}',
            'is_active' => true
        ],
        [
            'lesson_id' => 'css-intermediate',
    ...
    try {
        ...
        echo "⚠️  Could not clear existing challenges: " . $e->getMessage() . "\n\n";
    }
